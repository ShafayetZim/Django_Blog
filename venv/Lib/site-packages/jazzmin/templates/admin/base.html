    ...
    {% if not is_popup %}
        <footer class="main-footer {{ jazzmin_ui.footer_classes }}">
            <div class="float-right d-none d-sm-inline">
                <b>{% trans 'Ekattor it' %}</b>
<!--                <b>{% trans 'Ekattor it' %}</b> {% get_jazzmin_version %}-->
            </div>
            {% autoescape off %}
                <strong>{% trans 'Multi International' %} &copy; {% now 'Y' %} {{ jazzmin_settings.copyright }}.</strong> {% trans 'All rights reserved.' %}
            {% endautoescape %}
        </footer>
        {% if jazzmin_settings.show_ui_builder %}
